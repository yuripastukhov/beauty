...
Процедура ОбновитьПланировщик(ДополнительныеПараметры = Неопределено, Результат = Неопределено) Экспорт
	...
	ИзмерениеКлиент       = ИзмеренияПланировщика.Добавить("Услуга"); 
		//ВыборкаМастер = ВыборкаКлиент.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаМастер.Следующий() Цикл
		...
		//
		ВыборкаКлиент = ВыборкаМастер.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаКлиент.Следующий() Цикл
			Если ИзмерениеКлиент.Элементы.Найти(ВыборкаКлиент.Услуга) = Неопределено Тогда
			НовоеИзмерение = ИзмерениеКлиент.Элементы.Добавить(ВыборкаКлиент.Услуга); 
			НовоеИзмерение.Текст = ВыборкаКлиент.Услуга;                        
			КонецЕсли;	
			...
			Пока Выборка.Следующий() Цикл 
				...
				СоответствиеЗначений.Вставить("Услуга",   Выборка.Услуга);
				
				НовыйЭлемент = Планировщик.Элементы.Добавить(ДатаНачала, ДатаОкончания); 
				НовыйЭлемент.Текст = СокрЛП(Выборка.Клиент); 
...
Процедура ПланировщикЗаписейПередСозданием(Элемент, Начало, Конец, ЗначенияИзмерений, Текст, СтандартнаяОбработка)
	...
	Запись.Вставить("Услуга", ЗначенияИзмерений["Услуга"]); 
