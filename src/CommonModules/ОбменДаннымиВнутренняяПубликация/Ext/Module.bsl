...
Процедура УдалитьУстаревшиеЗадачи(Сценарий = Неопределено, РучнойОбмен = Ложь)
	...
	Исключение
		...
		Событие = НСтр("ru = 'Обмен данными.Удаление записей регистра задач';
						|en = 'Data exchange.Records deleted from task register'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			Шаблон = НСтр("ru = 'Не обнаружено активных задач обмена в корреспонденте. 
                                ...
                                |For information, see the event log in the peer infobase (%1).'", ОбщегоНазначения.КодОсновногоЯзыка());
