...
Процедура СохранитьДанныеВФайлПроверитьРасширение(РезультатВыполнения, ВходящийКонтекст)
	...
	ТекстСообщения = НСтр("ru = 'Для продолжения работы требуется установить расширение для работы с файлами.';
							|en = 'To continue, install a file management extension.'", КодЯзыка());
...
Функция ПолучитьРезультатВыполненияВФоне(ДлительнаяОперация, ВходящийКонтекст = Неопределено) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		Ответ.Ошибка = НСтр("ru = 'Работа сервиса DSS завершилась с ошибкой.';
	...
	Иначе
		Ответ.Ошибка = НСтр("ru = 'Вызов API сервиса DSS завершился с ошибкой при выполнении.';
