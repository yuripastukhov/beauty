	...
	Если ЗначениеЗаполнено(ДанныеДоверенности.НомерДоверенности)
		...
		РезультатПроверкиВРеестреФНС = ПолучитьЧастичныеДанныеДоверенностиМЧДРР(
			ДанныеДоверенности.НомерДоверенности,,НСтр("ru = 'Не удалось проверить доверенность';
														|en = 'Не удалось проверить доверенность'"));
...
Функция ОписаниеОшибкиHTTP(КодСостояния)
	...
	ИначеЕсли КодСостояния < 400 Тогда
		Возврат СтрШаблон(
			НСтр("ru = 'Ошибка по перенаправлению запроса с кодом %1';
				|en = 'Request redirection error %1'"),
	...
	ИначеЕсли КодСостояния = 503 Тогда
		Возврат НСтр("ru = 'Сервер распределенного реестра временно недоступен';
					|en = 'Distributed ledger service unavailable'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ДатаТранзакции) Тогда
			ОписаниеСостояния = СтроковыеФункции.ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">Запрос отправлен %2</span>';
					|en = '<span style=""color: %1"">Request was sent on %2</span>'"), "ПоясняющийТекст", ОтформатироватьДату(Выборка.ДатаТранзакции)));
		...
		Иначе
			ОписаниеСостояния = СтроковыеФункции.ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '<span style=""color: %1"">Запрос не направлялся</span>';
				|en = '<span style=""color: %1"">Request was not sent</span>'"), "ПоясняющийТекст"));
		КонецЕсли;
		...
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Ошибка
			И Выборка.ТипТранзакции = Перечисления.ТипыТранзакцийСРеестромМЧД.Регистрация Тогда
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '<span style=""color: %1"">: <a href = ""%2"">Ошибка регистрации</a>';
				|en = '<span style=""color: %1"">: <a href = ""%2"">Registration error</a>'"), "ПоясняющийТекст", "ПоказатьОшибку");
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Ошибка
			И Выборка.ТипТранзакции = Перечисления.ТипыТранзакцийСРеестромМЧД.Отмена Тогда
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '<span style=""color: %1"">: <a href = ""%2"">Ошибка отмены</a>';
				|en = '<span style=""color: %1"">: <a href = ""%2"">Cancellation error</a>'"), "ПоясняющийТекст", "ПоказатьОшибку");
...
Функция АвторизоватьсяНаСервереМЧДРР(ЦельАвторизации = "") Экспорт
	...
	Если СвойстваСервераМЧДРР.ИспользоватьРасширенияAPI Тогда
		...
		Если ТикетАутентификацииИлиДанныеПользователя = Неопределено Тогда
			ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Не удается авторизоваться на сайте интернет поддержки';
															|en = 'Не удается авторизоваться на сайте интернет поддержки'"), ЦельАвторизации);
			ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(, , , , ЗаголовокОшибки, НСтр("ru = 'Не удается получить тикет аутентификации или данные пользователя.';
																						|en = 'Не удается получить тикет аутентификации или данные пользователя.'"));
	...
	Исключение
		...
		ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Не удается авторизоваться на сервере Распределенного реестра ФНС России';
														|en = 'Не удается авторизоваться на сервере Распределенного реестра ФНС России'"), ЦельАвторизации);
	...
	Исключение
		...
		ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Отказ при получении токена доступа на сервере Распределенного реестра ФНС России';
														|en = 'Отказ при получении токена доступа на сервере Распределенного реестра ФНС России'"), ЦельАвторизации);
	...
	Если НЕ ЗначениеЗаполнено(Результат.ТокенДоступа) Тогда
		...
		ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Распределенный реестр ФНС России не предоставил токен для авторизации';
														|en = 'Распределенный реестр ФНС России не предоставил токен для авторизации'"), ЦельАвторизации);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не предоставил токен для авторизации.';
				|en = 'Распределенный реестр ФНС России не предоставил токен для авторизации.'"));
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьНомерМЧДРР(ТокенДоступа = "") Экспорт
	...
	Исключение
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, 
			НСтр("ru = 'Не удается получить номер доверенности';
				|en = 'Cannot get the letter of authority number'"), ПредставлениеОшибки);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецПопытки;
	...
	Если НЕ ЗначениеЗаполнено(Результат.НомерДоверенности) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не выдал номер доверенности.';
				|en = 'The distributed ledger of the Federal Tax Service of Russia did not provide the letter of authority number.'"));
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьСтатусТранзакцииМЧДРР(ИдентификаторТранзакции, ТокенДоступа = "", НомерДоверенности = "", ЦельПолучения = "") Экспорт
	...
	ЗаголовокОшибки = ?(ЦельПолучения = "", НСтр("ru = 'Не удалось обновить статус транзакции';
												|en = 'Не удалось обновить статус транзакции'"), ЦельПолучения);
	...
	Если НЕ ЗначениеЗаполнено(Результат.ИдентификаторТранзакции) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не вернул данные о статусе транзакции.';
				|en = 'Распределенный реестр ФНС России не вернул данные о статусе транзакции.'"));
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьЧастичныеДанныеДоверенностиМЧДРР(НомерДоверенности, ТокенДоступа = "", ЦельПолучения = "") Экспорт
	...
	ЗаголовокОшибки = ?(ЦельПолучения = "", НСтр("ru = 'Не удалось получить частичные данные доверенности';
												|en = 'Не удалось получить частичные данные доверенности'"), ЦельПолучения);
	...
	Если НЕ ЗначениеЗаполнено(Результат.ХешДоверенности) И НЕ ЗначениеЗаполнено(Результат.НомерДоверенности) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета , ДополнительныеФайлы, ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не предоставил частичных данных доверенности.';
				|en = 'The distributed ledger of the Federal Tax Service of Russia did not provide partial data of the letter of authority.'"));
		ПараметрыОшибкиМЧДРР.ПараметрыТекстаОшибки.Вставить("НомерДоверенности", НомерДоверенности);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ТокенДоступа = "", ЦельПолучения = "") Экспорт
	...
	ЗаголовокОшибки = ?(ЦельПолучения = "", НСтр("ru = 'Не удалось получить полные данные доверенности';
												|en = 'Не удалось получить полные данные доверенности'"), ЦельПолучения);
	...
	Если НЕ ЗначениеЗаполнено(Результат.СтатусПолучения) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗапросHTTP, ОтветHTTP, СтруктураОтвета, ДополнительныеФайлы, ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не предоставил полных данных доверенности и не сообщил о статусе запроса данных.';
				|en = 'The distributed ledger of the Federal Tax Service of Russia did not provide complete letter of authority data and data request status.'"));
		ПараметрыОшибкиМЧДРР.ПараметрыТекстаОшибки.Вставить("НомерДоверенности", НомерДоверенности);
		Результат.Ошибка = ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибкиМЧДРР);
		Возврат Результат;
	КонецЕсли;
...
Функция ЗагрузитьДоверенностьИзРеестра(НомерДоверенности, ИННДоверителя, ТокенДоступа = "") Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось загрузить доверенность';
							|en = 'Не удалось загрузить доверенность'");
...
Функция ОтменитьМЧДРР(ИмяФайлаВыгрузки, ДанныеИлиАдресВыгрузки, ДанныеИлиАдресПодписи, Доверенность, ТокенДоступа = "") Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось отменить доверенность';
							|en = 'Не удалось отменить доверенность'");
	...
	Если Не ТипЗнч(ДанныеДоверенности) = Тип("Структура") Тогда
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru = 'Не поддерживается отзыв с типом доверителя:';
								|en = 'Revocation with the principal type is not supported:'") + ДанныеДоверенности.ТипОрганизации;
