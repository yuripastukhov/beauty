		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если ЭтоМодельСервисаСДоступнымУсовершенствованием Тогда
				...
				Элементы.ТипПодписиКриптографииПоУмолчаниюРасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
					...
					|en = 'In the web app, archival signatures are unavailable by default. You can select this signature type when signing with a <a href=%1>certificate</a> installed on the computer with a <a href=Программы>digital signing app</a>.'"), "Сертификаты");
			Иначе
				...
				Элементы.ТипПодписиКриптографииПоУмолчанию1РасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
					...
					|en = 'In the web app, the default signature type is ""Basic"". You can select signature types with timestamps when signing with a href=%1>certificate</a> installed on the computer with a <a href=Программы>digital signing app</a>.'"), "Сертификаты");
			КонецЕсли;
		...
		Если ПредставлениеПодписи = "" Тогда
			ПредставлениеПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подпись с меткой доверенного времени (%1)
					 ...
					 |en = 'Подпись с меткой доверенного времени (%1)
	...
	Если Место = "Сервер" Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удалите сертификат из локального хранилища сервера %1.';
				|en = 'Delete the certificate from the local store on server %1.'"), ИмяКомпьютера());
