...
Процедура ДоступностьПризнакаРегистрацииВРеестре(Форма)
	...
	Если Форма.Элементы.РегистрироватьВРеестре.ТолькоПросмотр 
		...
		Форма.Элементы.РегистрироватьВРеестре.Заголовок = НСтр("ru = 'Зарегистрирована в Распределенном реестре ФНС России';
																|en = 'Registered in the distributed ledger of the Federal Tax Service of Russia'");
	Иначе
		Форма.Элементы.РегистрироватьВРеестре.Заголовок = НСтр("ru = 'Регистрировать в Распределенном реестре ФНС России';
																|en = 'Register in the distributed ledger of the Federal Tax Service of Russia'");
	КонецЕсли;
	...
	Иначе
		...
		Если КоличествоПолномочийВЗаголовке = 0 Тогда
			Форма.Элементы.ДекорацияПолномочияПредставителя.Заголовок = 
				...
				НСтр("ru = 'Машиночитаемые полномочия (%1)...';
					|en = 'Machine-readable powers (%1)...'"), Форма.Объект.Полномочия.Количество());
		Иначе
	...
	Иначе
		Форма.Элементы.ЛицоДоверителяТипДекорация.Заголовок = ?(Форма.ЛицоДоверителяРеквизиты.ЭтоФизическоеЛицо,
			НСтр("ru = 'Физическое лицо';
				|en = 'Физическое лицо'"), НСтр("ru = 'Организация';
												|en = 'Организация'"));
	КонецЕсли;
