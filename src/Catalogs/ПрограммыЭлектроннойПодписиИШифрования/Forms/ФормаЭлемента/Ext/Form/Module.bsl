	...
	Если ВыбранноеЗначение = ПредопределенноеЗначение(
		...
		Если ПоставляемаяПрограмма = Неопределено Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Программа с указанным именем и типом не может быть определена автоматически.';
											|en = 'An application with the specified name and type cannot be determined automatically.'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьЗаголовокАвтоматическиеНастройки(ПоставляемаяНастройка)
	...
	Если ПоставляемаяНастройка = Неопределено Тогда
		Элементы.ДекорацияНадписьАвтоматическиеНастройки.Заголовок = НСтр("ru = 'Эта программа не может быть определена автоматически';
																			|en = 'This application cannot be determined automatically'");
		Возврат;
	КонецЕсли;
	...
	Элементы.ДекорацияНадписьАвтоматическиеНастройки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Алгоритм шифрования: %3';
			|en = 'Algorithms used in this application:
			|Signing algorithm: %1
			|Hashing algorithm: %2
